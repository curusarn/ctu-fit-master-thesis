...
Junior data analyst Mark has recently started his first job. He uses shell because he needs it for his job. 
...
In this first category, we consider workflows that involve retrieving \textit{recent} history entries. In this context, recent means that the user remembers the command. \redtext{In all of these workflows the user also wants to retrieve the full history entry and execute it without any editing.}

% \todotext{MAIN: recalling recent and semi-recent commands "as exact matches", "recent" ~ the user remembers the command}


The first workflow is recall of a very recent history entry; In this workflow the user remembers the exact position of the command line entry. An example of such workflow is running the following sequence of commands: 
\begin{verbatim}
make build
vim hello_world.c
make build
\end{verbatim}
First, we build the project using \verb|make|. Then we edit the file using \verb|vim|. After that, we press \verb|ARROW_UP| twice and press \verb|ENTER| to execute the recalled history entry. 
...
When recalling very recent commands users often remember the exact position of the history entry; This means that they do not read the history entry that appears on the command line but they blindly execute it instead. This is an efficient workflow that shows us the importance of the order of history entries.



\todotext{USECASE: recall a recent command - press arrow up 5-6 (3-8) times; }

\todotext{We saw that even someone who claimed to not use arrow up repeatedly does so. It seems that history entry can be perceived as "recent" and people will use arrow up to recall it despite being 8 entries away. The behaviour we observed also suggests that people often remember the exact command line entry they executed earlier despite it not being very "recent" history wise.}



\todotext{type prefix + press arrow up 2-3 times NOTE: recent in time vs. recent in command sequence}
\todotext{similar, more effective way to do so. We noted earlier that it seems that users usually have a very good idea of what history entry they are looking for when recalling recent commands.  This means that it is reasonable to assume that the user remembers the command stub and could use it to recall the recent history entry. }


\subsubsection*{Editing recalled commands}
%\subsubsection*{Fixing recent mistakes} ???


\todotext{USECASE: fix a typo in a previous command - press arrow up, edit }

\todotext{USECASE: use: sudo !! to run a previous command with sudo}

\todotext{USECASE: (edit) reuse a portion of a recent command - press arrow up like 5 times, edit }
