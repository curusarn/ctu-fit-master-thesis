...
\begin{itemize}
    \item 
    \item In this work we set out to improve the standard shell history. 
    \item We want to design and implement a solution that solves issues of std. history and uses context to enhance its capabilities.
    \item ??? We collect shell history; We analyze and compare it with previous research about shell usage and shell history tools. - We found out that potential for hsitory tools is 4x the 90s and ...
    \item We analyze standard history features. We explore common issues people have. We describe typical workflows people do and shortcoming of std. hitsory tools. - We identify reverse search as inefficient (redesign)
    \item We analyze existing hisotry tools and find ones that solve the isses with reverse search. We also find contextual tools that solve "nothing". - Useful contextuial history tools need to match the improvements of state-of-the-art non-contextual tools.
    \item We describe available context and how it relates to history usage.
    \item We design a history system based on requirements and our analysis.
    \item We implement parts of the system that record history with context and use this context to show relevant history entries when searching shell history.
    \item Our solution always performs similarly or better than Hstr (s-o-t-a).
    \item Our solution performs much better than Hstr in "many" situations.
    \item Our solution requires less knowledge and typing when searching.
    \item Our solution on average saves more typed chars to the user. (effort by extension)
    \item Our solution has been installed over 600 times in last five months (since the initial release).
    \item People switched from Hstr and they are happy.
\end{itemize}
