...
Many command line entries are not executed individually; They are a part of longer sequences and tasks. %Each entry is a part of a sequence; It has preceding and following command line entries. 
...
Nevertheless, not all of the sequential contextual information is difficult to interpret and use. Sequences of history entries are useful because there are situations when people want to repeat them\redtext{REF: sequential workflow}. 
...
Each of the devices is at least a little bit different so it is a good idea to be able to tell them apart. The \verb|$HOST| environment variable is a good identifier of the device. We can use the host to prioritize the history from the current device and to explicitly show the user which host is the specific history entry coming from.
...
We could detect and handle these portability issues individually. Or alternatively we could take a simpler approach and treat all history entries from other devices differently than ones from the current device. This could be effective because many of the portability issues are related to mixing shell history from different devices.
