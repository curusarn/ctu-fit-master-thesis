...
\begin{figure}[]
  ...
  \caption{Screenshot of Hstr\todotext{TODO: title, TODO: make a better screenshot}}
...
These requests take time so there is no interactive "search as you type" functionality. Instead, you always have to type out the full query, execute the search command, and then wait\footnote{This takes about three seconds on my system.} for the results. Below, you can see an example of search that is restricted to the current directory. 
...
Another disadvantage is that the server-side search functionality requires 
server-side access to your history in open form (non-encrypted)

This design makes client side encryption impossible
To add insult to injury the server is closed source and the author of Bashub is running the official instance

% %%%%%%

 The server is provided by the author of the project.
Your history is saved on a remote server provided by the Bashhub maintainer. 
This means that you can access your history from all of your machines. 


\todotext{TODO: bashhub-server - new }
There is an additional open source implementation of Bashhub server available.
This makes it possible to self-host instead of trusting the someone with your shell history.


% bashhub library
\redtext{Mention https://github.com/rcaloras/bash-preexec ??? I don't think so}
