...
In all the workflows we discussed so far, the user wanted to recall a fairly recent history entry. Now, we move on to workflows where the user searches for history entries that are older. When people search for older history entries they usually only remember a limited amount of information about the desired history entry. 

Below, we see an example for this workflow; The upper part are relevant history entries and at the bottom we see the command line entry the user wants to find. %We chose to format the example this way because the full sequentiality of the history is no longer important when searching for older history entries. 
% Keep in mind that the user does not have the line in front of them like we do nor he does remembers the full command line entry.  
...
In the example above, the user wants to remotely login to a \verb|thumbnail api| server in development environment using \verb|ssh|. He does not remember the full command line entry he is searching for.
...
Here, the user sees that there are other history entries that match the query \verb|thumbnail|. He will make the query more specific by typing \verb|-api|.
...
Now, reverse search still does not show the desired history entry but it shows an entry with the correct server URL. The user thinks that the query is specific enough and presses \verb|CTRL+R| to show the next matching history entry.
...
The next result is the desired history entry. We just saw an example of using reverse search that was relatively smooth and straight forward; Unluckily this is not always the case. Consider, if the user started with a less specific query such as \verb|ssh|.

\begin{verbatim}
(reverse-i-search)`ssh': ssh 192.168.2.105
\end{verbatim}

This gives the user a very generic \verb|ssh| command. Extending the query to make it more specific is much harder in this situation; The user will not be able to extend the query, unless he remembers he was logging in under \verb|root|. The only way to make the query more specific is to get rid of it and start over. 

% The \verb|ssh| query is a good one; The user is certain that it is present on the desired history entry. 
