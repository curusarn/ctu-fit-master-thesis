...
\subsubsection*{Editing a single command line entry}

In this first workflow, the user submits a command line entry but is not satisfied with the result. To fix that, he retrieves the immediate previous history entry, modifies it and executes it again. We look at multiple example sequences of command line submissions for this workflow.

\begin{verbatim}
    wget https://pastebin.com/raw/EDELXNYp --silent
    wget https://pastebin.com/raw/EDELXNYp --quiet
\end{verbatim}
In this example, the user is trying to download a file from \verb|pastebin| using \verb|wget|. The first command ends with a non-zero exit status because \verb|--silent| is not an option of \verb|wget|. He presses \verb|ARROW_UP| to retrieve the command line entry, fixes the mistake and presses \verb|ENTER| to submit the command line entry again. 
...
Here, the user wants to install \verb|zsh| package. The command fails because superuser permissions are needed to install packages to the system. Instead of using interactive history features, the user uses csh-style history expansion \verb|!!| which repeats the previous history entry. 
...
We just saw multiple different situations in which people continue working on the same command. We also saw different approaches to retrieving previous history entries. It is worth noting that in all of these situations the user relies on the sequential property of history. 
...
This workflow is a recall of a very recent history entry; The user remembers the exact position of the history entry he wants to recall. An example of such workflow is running the following sequence of command line submissions: 
\begin{verbatim}
    make build
    vim hello_world.c
    make build
...
The usage data we collected shows that even somebody who claimed to not press \verb|ARROW_UP| excessively actually does so quite often. Pressing \verb|ARROW_UP| five times is more common than you would expect and pressing it over ten times is rare but it still happens. 
...
\paragraph{\redtext{TODO: next title, same workflow}}
...
This is a good example of how the number of keystrokes is not the only thing we should be taking into account; Generally speaking, shell history should save the effort and time of its users. This means both the physical typing and the cognitive overhead required to retrieve the entries.

\todotext{Is this good ending}

% BAD?: Retrieving command line entries from history should be easy in terms of both physically typing and  Ultimately, the goal of history facilities is to save effort and time.
...
% Everyone knows about arrow up, not everyone knows about prefix search but quite a few people praise it and claim to use it extensively

% Some people praise and claim to heavily rely on history prefix search.
%In contrast to using \verb|ARROW_UP| for stepping through history, 

% BAD: We did not see this in the data we collected but majority of our subjects did not know that prefix search exists or do not have it configured. We suspect that it is why ...

\subsubsection*{Recalling a recent sequence of command line entries}



\subsubsection*{Searching with limited knowledge}
\todotext{One difference here is that the history entries are no longer recent. This means that arrow up is not an effective way to retrieve them. But more importantly it means that the user no longer knows/remembers the whole command line entry they want to recall. This makes retrieving the history entry harder.}


\todotext{USECASE: find a long curl command}
