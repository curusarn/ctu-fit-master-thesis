...
The user found the desired history in the list and uses history expansion to execute it; He types \verb|!421| and presses \verb|ENTER|. This seems pretty quick and efficient. What if we cannot not spot the desired history entry in the list of results?

In such case, nothing prevents us from further filtering the results. Pressing  \verb|ARROW_UP| retrieves the previous command line entry and typing \verb#| grep thumbnail# adds another query. We can repeat this until we are happy with the filtered results.
...
We see that manual filtering has clear advantages over reverse search. 
First of all, we do not need a very specific query because we are able to quickly scan many simultaneously displayed history entries. 
The \verb|ssh| query that was not specific enough in reverse search gave us some useful results when filtering manually.

Additionally, in situation when the query is not enough, it is easy to combine multiple queries together. This makes manual filtering much more powerful than the reverse search. 
When using the reverse search we had to choose between the queries. Here we can use all of them; This results in a very flexible workflow that does not needlessly restrict the user.
