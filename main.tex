...
\begin{itemize}
    ...
    \item Provide good out-of-the-box experience \redtext{(too high-level?)}
    \item Respect existing habits of people \redtext{(too high-level?)}
...
\[ Score_r(q,c) = QueryScore_r(q) + ContextScore_r(c) + TimeScore_r \]
...
Specific weights used in the partial scores should be determined experimentally based on real life shell history.
...
In this section, we test out design to make sure that we covered all the requirements.


\begin{itemize}
    \item Record shell history with context and usage ... History collector records the history with context and usage and sends it to the daemon.
    \item Allow history sharing between open sessions ... Daemon handles all of the history so it can send it to any component regardless of sessions.
    \item Provide good out-of-the-box experience ... Our design does not expect people to configure it before using it. Designed behaviour is based on the previous research and modeled target users.
    \item Respect existing habits of people ... We redesigned the behaviour of standard history while keeping the original use-cases intact. Recent history is still accessible on \verb|ARROW_UP| in unchanged order. People can still press \verb|CTRL-R| for general purpose history searching. 
    \item Provide a replacement for reverse search ... Our history searching application provides features that superset the reverse search. It can search anything that reverse search can because the query is designed to have the most influence over the scoring function.
    \subitem Solve the issues with standard reverse search ... Reverse search only uses a single-query and shows a single-result. This significantly reduces its usefulness. Our searching app uses multiple word queries and shows a page full of results.
    \subitem Match the improvements offered by Hstr and Fzf ... dtto. The main improvement is the multi-query, multi-result search. ???
    \subitem Use recorded context to enhance history searching capabilities ... The search application searches history based on both query and current context.
    \item Provide support for sequence repeating (forward in history) ... Forward in history is a feature that makes it much easier to repeat sequences.
    \item Provide support for synchronization of shell history between devices ... Synchronization between devices is make possible because of the daemon and the synchronization connectors.
    \item Provide support for contextual autosuggestions ... Autosuggestions handler provides support for contextual autosuggestions.
\end{itemize}
...
\begin{figure}
\centering
  \tmpframe{\includegraphics[width=\linewidth]{figures/implementation/thesis-impl-search-app-data-lifetime.pdf}}
  \caption{Schema of search and rendering process of the search application}
  \label{impl-search-app-data-lifetime}
\end{figure}
...
\centering
  ...
  \caption{Schema of internal daemon structure (async vs sync)}
...
\redtext{USER FEEDBACK}

turn off context -> raw mode

use ctrl+R to get next result -> collision with 
