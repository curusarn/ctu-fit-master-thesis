...
Host and directory also share a single column but they do have a separate headers. Columns are being dynamically resized to fit the data and to not waste space. Command line entry, host, and directory are shortened to fit the screen if necessary. 
...
\paragraph{Design from 80x25 to fullscreen}
...
\[score(record) = queryScore(query, commandLine) + contextualScore(currentContext, recordContext) \]
\[ queryScore(query) > contextualScore(context); \forall  \]
