...
When the user uses \verb|ARROW_DOWN| the next autosuggestion should try matching a few next history entries in the sequence before any  other history entries. 
...
It is essential that the query is given more weight in the score then the context. Not doing so could lead to situations where the user types in a query but the displayed results match the context instead.

\begin{itemize}
    \item R ... set of all records
    \item Q ... set of all queries
    \item C ... set of all contexts
    \item S(q, c, r) ... total score of given query, context, and record
    \item \(S_q(q, r)\) ... query score of given query, and record
    \item \(S_c(c, r)\) ... context score of given context, and record
    \item \(S_t(r)\) ... time score of given record
    \item \(r = (text, time, ctx)\) ... (for purpose of these formulas) each record consists of command line, time, and context
    \subitem \(text_r\) ... command line entry of record \(r\)
    \subitem \(time_r\) ... date/time of record \(r\)
    \subitem \(ctx_r\) ... context of record \(r\)
\end{itemize}
...
For any queries \(q_1\), \(q_2\) and history record \(r\):

\[ length(q_1) > length(q_2) \Rightarrow QueryScore_r(q_1) \geq QueryScore_r(q_2)\]\
...
% something somethign
We want the records to be grouped based on context, to achieve that 
We want to prevent history records with different contexts from unnecessarily mixing together. 
To achieve that, \(ContextScore\) should be an injective function in respect to contex of records.

In other words, if two records share the same \(ContextScore\) they also must share the same context:

\[ ContextScore_r(c) = ContextScore_s(c) \Rightarrow context_{r} = context_{s} \]\

In other words, any two records with different contexts never share the same \(ContextScore\):
...
\begin{tabular}{lll}
\hline \hline
Context condition      &  \multicolumn{2}{c}{Influence on score} \\\hline
directory matches      & positive & large   \\ 
git repository matches & positive & medium \\ 
non-zero exit status   & negative & small   \\
host does not match    & negative & minor   \\ 
\hline \hline
\end{tabular}
\caption{Influence on score of different parts of context}
