...
\begin{figure}[h!]
  \tmpframe{\includegraphics[width=\linewidth]{figures/existing-tools/fish-autosuggestions-screenshot.png}}
  \caption{Fish shell autosuggestions}
  \label{fish-autosuggestions}
\end{figure}

\todo{CHECK \hatxxx Is this resolution enough?}
...
\begin{figure}[]
  \tmpframe{\includegraphics[width=\linewidth]{figures/existing-tools/hstr-screenshot.png}}
  \caption{Hstr interactive history search\todotext{TODO: title, TODO: make a better screenshot}}
  \label{hstr-screenshot}
\end{figure}


\begin{figure} \tmpframe{\includegraphics[width=\linewidth]{figures/existing-tools/hstr-screenshot-100cols.png}}  \caption{Example Hstr screenshot 100 \todotext{TODO: REMOVE}} \end{figure}

\begin{figure}   \tmpframe{\includegraphics[width=\linewidth]{figures/misc/resh-cli-screenshot.png}}   \caption{Example Resh cli screenshot 87 \todotext{TODO: REMOVE}} \end{figure}
...
\begin{figure}
  \tmpframe{\includegraphics[width=\linewidth]{figures/existing-tools/fzf-fh-screenshot.png}}
...
These requests take time so there is no interactive "search as you type" functionality. Instead, you always have to type out the full query, execute the search command, and then wait\footnote{Bashub server response takes about three seconds on my system.} for the results. Below, you can see an example of search command that is restricted to the current directory. 
...
Recently\footnote{Bashhub-server was written in February 2020.} a new open source implementation\cite{toolsbashhubserver} of the Bashhub server has appeared. This addresses the privacy and security issues by making it possible to run and use your own instance of the server.
...
McFly\cite{toolsmcfly} is a tool that records your shell history with context. 

Then it tries to predict the next command line entry based on current context using a small neural network.

McFly predicts history entries based on following contextual information:
\begin{itemize}
    \item present working directory
...
Unlike Bashhub, this tool is designed to be bound to \verb|CTRL-R| and to replace the standard reverse search. Pressing \verb|CTRL-R| launches McFly full screen terminal app. When the app is launched it displays a list of ten predicted history entries. The list of history entries is updated as you type. McFly only recommends history entries that exactly match the typed query including spaces.

\begin{figure}[]
  \tmpframe{\includegraphics[width=\linewidth]{figures/existing-tools/mcfly-screenshot.png}}
  \caption{McFly interactive history search \redtext{TODO: better screenshot with more history and hopefully better colors}}
\end{figure}

When trying out this tool we found its behavior to be unpredictable; Not knowing why specific results are being shown made the tool less useful. 

Like reverse search, it only uses a single query for searching; This makes it hard to retrieve in situations when we are unable to think of a better query\footnote{We described such situation earlier in \redtext{REF: reverse search workflow}}. 


McFly is popular on GitHub.
\footnote{McFly has sixteen hundred "stars" on GitHub.}
