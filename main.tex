...
Directories provide explicit context; People change into different directories to complete different tasks. It is often more comfortable to change directories instead of using longer paths as arguments. 
...
Git remotes\footnote{Remotes are remote repositories tracked by Git. Origin is the default remote.} can be used to identify the repository even across different machines or when it is moved. 
...
Most command line entries are executed in relation to other command line entries.
Each history entry is a part of a sequence; It has preceding and following command line entries. 
...
It is unclear how to use this complex contextual information about relationships between commands to provide value to the user. It is beyond the scope of this work to study the relationships between history entries and between sessions.


However, not all of the sequential contextual information is difficult to interpret and use. Sequences of history entries are useful because there are situations when people want to repeat them\redtext{REF: sequential workflow}. 


Having access to a full transcript of your actions also has its value. It gives you the ability to refer back to what you were doing earlier. A transcript of command line entries should also obviously include the time of execution of the individual history entries.
% NOT an auditing tool

\subsection{Host and its context}

this all starts making a lot of sense if you start syncing your history

Host

offer history from current host first but make it still easy to get history for current directory/git from all hosts

Os
distro
shell

Not all commands are compatible - different distros use different package managers
not all shell commands are compatible

for example there could be simple rules that penalize package manager history entries on incompatible distros

To take this a step further / there is more host context that makes commands incompatible

shell configuration ??? what did I mean by this
aliases
variables ? - privacy issues ?

installed programs

paths (absolute vs relative) - is this related enough - not host specific but compatibility related
