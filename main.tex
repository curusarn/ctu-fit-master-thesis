...
\subsubsection*{Command Vocabulary}
...
% >>> Top 100 %% of cmds amounts for 100.0 %% of all command lines
% >>> Top 10 %% of cmds amounts for 63.43728659230594 %% of all command lines
% >>> Top 20 %% of cmds amounts for 71.34987480081949 %% of all command lines
...
\subsubsection*{Sequential relationships between commands}
\todotext{TODO: write the chapter}
...
"Through a multivariate analysis of UNIX commands invoked
by the site population, Hanson, Kraut, and Farber (1984) examined the interaction
effects between commands. Their results show statistically significant relationships
between certain command chains; the relations between the fifty most frequently
used commands are shown in Figure 3.3. Each ball in the network represents
a command, its size indicates the usage frequency, and the arrow indicates the
significant dependencies. One dimension of these relationships is modularity.
Some commands, such as Is, are core commands - they are used frequently and
are surrounded by many other commands (i.e., highly modular and independent).
Others are not; they are surrounded by specific command sequences. An example
of the latter is cp, which is generally preceded by itself and followed by chmod."

"Commands are also related by functional clusters, such as editing, process management, orientation, social communication, and so on (Hanson, Kraut, and Farber,
1984), which may not be revealed by statistics."


Greenberg concluded that assuming that the sequential dependencies of commands found by polling all the users together is a mistake. 
...
We created the graph by taking N most used commands. Each node represents a command and the size of the node is proportionate to the command frequency. Each vertex represents a \redtext{transition chance} from one command to the next one. We decided to create the graph this way because it better represents properties that could be potentially used in history tools. Because it only uses the probability of transition from one command to the next one whereas in the method used by Hanson also considers the reverse transition probability; That is the probability that given command was preceded by the previous command. For example, when predicting the next command we only have the information about the past and current command. 

\todotext{TODO: Table with arrowstyles, arrowheads, and transition probabilities.}
...
We can see some commands from which there are no verteces; This could be caused by user closing the session


\redtext{toolsmith and the original sequence graph talked about how individuals have }

\todo{Summed up toolsmith}
...
\begin{figure}
  ...
  \caption{Sequential structure of command usage. Legend: \ref{tab:seq-table} }
  \label{seq-graph}
\end{figure}
...
Regular/full     & 100 - 30\% (specified by labels) \\
Dashed      & 30 - 20\%                         \\
Dotted      & 20 - 10\%                         \\
\hline
\end{tabular}
\caption{Legend for figure \ref{seq-graph}}
\label{tab:seq-table}
\end{figure}
