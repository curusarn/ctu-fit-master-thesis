...
Experienced professional programmer \textit{Peter} develops, deploys, and operates back-end applications.
...
# desired command line entry:
    ssh root@thumbnail-api.dev.example.com
\end{verbatim}
\todo{CONSIDER removing "relevant history entries" from this example based on how it ends up laid out on the page}

As in the previous examples, the user wants to remotely login to a \verb|thumbnail api| server in development environment using \verb|ssh|. He uses \verb|history| built-in to print the shell history together with the \verb|grep| command to filter the history entries.
...
Here, we see a section of shell history related to a specific project. The user executed all of these command line entries a long time ago in the \verb|~/git/image_server/| directory. He did not work on the \verb|image_server| project since. This means that he does not remember these history entries and his history is filled with many other command line entries he executed since.

The user comes back to the project by typing and executing \verb|cd ~/git/image_server|\todo{Overflow}. He wants to continue working on the project. He knows that he needs to edit some code, build the project, test it, and release a new version. However, he does not remember concrete command line entries for these tasks. 
...
First, important history feature we discuss are autosuggestions. As you type on the command line autosuggestions suggest a single history entry that starts with characters you already typed. The suggested text is displayed in grey as shown in \redtext{TODO: REF}. 

\todotext{IMAGE - SCREENSHOT of FISH autosuggestions}

This history feature is very quick and convenient to use. You can use the shell as you would normally with the option to accept the suggestion whenever it matches what you want to type. Autosuggestions can replace neither \verb|ARROW_UP| nor proper history search; However, they work well in combination with other history mechanisms.

Autosuggestions also have some interesting properties compared to \verb|ARROW_UP|.\todo{Overflow}
When people use simple \verb|ARROW_UP| they often expect to retrieve immediately recent history entries\footnote{We described a specific workflow showing this in previous section \redtext{REF}.}.  This is not the case with autosuggestions. The displayed suggestion can be the most recent matching history entry but it also does not have to be. Autosuggestions can use more experimental and more advanced recommendation techniques.
...
\subsection{History features of (Python) Read-eval-print loop}

python in blender


\todo{I don't have this in python REPL anymore!!! Did they switch to gnu readline ???}


\subsection{Multi-query multi-result history search \redtext{TODO: better title?}}

\redtext{Actually write this vvv}
When we talked about reverse search we identified two issues with it. First, it only uses a single query for searching. And second, it only displays a single result.

Enter Hstr. Hstr is an interactive history searching tool that addresses both of these issues. Plus it does not require the extra typing like manual history filtering. It is intended to fully replace the reverse search.

When launched it displays a fullscreen app as shown in the figure. You type a query at the top and below you see a page of matching history results. Each word of the query you typed is used as a separate keyword when searching, by default.

The app also includes a few options such as exact search, regex search. The keyword search is the default and the most convenient mode. 

Hstr is very popular.
Some of the people who use hstr describe it as life changing.
...
\todo{light there screenshot}

\subsection{Fuzzy history search}

Similarly to Hstr, fuzzy search also addresses the issues of the reverse search.

Fzf is a multipurpose fuzzy finder. It is a very popular way to search history.

As Hstr, It can display a full page of results from history.

Instead of just taking each word of the query as a separate keyword. Fuzzy search takes it a step further. It gives you the more exact matches first but if there are no proper matches, it returns even the results that do not properly resemble the query.

This is an effective way to return meaningful results even in cases when you make typos.

Again, Fzf is very popular.
As with Hstr, many people cannot imagine switching away from fzf. 
...
\todotext{TODO: bashhub-server - new }
...
Issues with performance - python

Issues with trust - closed source
...
\subsection{Other contextual history tools and their issues}

\todotext{TODO: advanced-shell-history - contextual history with smart querying - too buggy to be useful}

\todotext{TODO: rash - python again, explicit search by context is a nice to have but not really a workflow }
...
% meh, I'm not sure about this structure
