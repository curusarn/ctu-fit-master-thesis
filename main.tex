...
\todo{TRY: some package for verbatim in footnotes}
...
Executing \verb|fc -4 -1| opens an editor with last four history entries and then, closing the editor executes the commands.  This, however, is quite cumbersome in practice and not many people seem to use it. %\todo{\verb||!-4 works in a similar way}
...
The next result is the desired history entry. This was a pretty efficient but also a quite idealistic reverse search example. It worked out so well because the user was able to provide a specific enough query. We should look at a more realistic situation where the user struggles to provide the query. %We will use the same shell history and desired command line entry as in the previous case. (same sentence as the start of the ne)
\paragraph{\redtext{TODO: title "Struggling with reverse search"}}
...
Here, we see how the user struggles to make the query more specific; Extending the query requires a knowledge of details about the desired command line entry. 
...
This result looks better than the generic history entry from before. However, it is still far from what we are looking for. The user should extend the query to make it more specific. 

In the previous example, we assumed that the user remembers enough to extend the query to \verb|thumbnail-api|; That is a bold assumption. What if the user does not remember that words \verb|thumbnail| and \verb|api| are adjacent or that they are delimited by a dash symbol? In practice, it is often hard to come up with a query that does not match many other history entries. 

To go through the user repeatedly presses \verb|CTRL+R| to display the next result. At first, the user presses \verb|CTRL+R| slowly to have enough time to read the results. After about three keypresses he becomes impatient and speeds up. This does not leave enough time to properly read the results and he presses \verb|CTRL+R| one too many times.
...
At this point, the user gets annoyed because the only option is to start over. He aborts the search, presses \verb|CTRL+R|, and types in \verb|thumbnail|. Then he presses \verb|CTRL+R| five times; This time he is more careful to not go too far again.




% The user once again struggles to extend the query
% -> The user will press Ctrl+R repeatedly instead of making the qeury more specific
...
% This is not ideal because \verb|ssh| 

\todo{TODO: Rewrite}
People generally do not like to start over. To avoid deleting the query we could repeatedly press \verb|CTRL+R| until we find the desired history entry. This is however not without its own downsides and limits.

\todo{MOVE some of this up to previous chapters? make it shorter here or move it altogether}
\redtext{start}
When pressing \verb|CTRL+R| repeatedly we are forced to read the history entries one by one which is quite inefficient. Additionally, pressing \verb|CTRL+R| too many times can be very punishing because it is not always possible to "go back" to previous history entries; Keybinding \verb|CTRL+S| should allow you to "go back" while in reverse search. However, by default\footnote{This is true for default configurations of bash supplied in many popular distributions.}, it will freeze all input coming to your terminal making it seemingly unresponsive. This is a good example of how common default configuration can be extremely unfriendly to new users. 
\redtext{end}

\todo{Somehow break this long section HERE}
...
% The other option is making the query more specific; This is not always easy. Consider the example above; Query \verb|ssh| matches five history entries, \verb|thumbnail| matches seven, and \verb|api| matches four. 

% A better query would be \verb|thumbnail-api| but this opens the opportunity for mistakes such as \verb|thumbnail_api|. We see that even when we see the history entries it is not easy to come up with a good query. 
...
PROBLEM: Too many results: we need more specific query
NOTE: The user only remembers: curl to github + they could probably remember some of the options ...
PROBLEM2: reverse search supports one continuous query - BUT the user remembers parts of the command, not a continuous query
...
% repeated
%TODO: Too many curl commands. Prefix search is no longer enough. User remembers bits of the command line entry. Reverse search suffers from similar disadvantages.
%Describe how if you remember two bits of info std. interactive options are no good
...
% In these workflows the difference is
...
\end{verbatim}
\todo{CONSIDER: shortening the iso path}
...
% prev
Instead of relying on the knowledge of the user these workflows use available implicit contextual information.
This workflow assumes that the user does not remember additional information that could be used.
...
\subsubsection*{Explicit contextual search}
\todotext{NOTE: Nobody actually wants this?}

\todotext{USECASE: search history from remote host ???}

\todotext{USECASE: search by directory + search by command ???}
