...
\begin{itemize}
    \item Describe features of standard shell history
    \item We introduce typical shell users we design for
    \item We formulate typical workflows for those users. Describe the workflows in detail, identify which are easy to achieve with std. shell history features and which are difficult or have issues
    \item Explore existing shell history tools. Identify which issues of std. shell history they solve. 
    \item Examine contextual information available in shell. Explain how it could be useful in history tools.
\end{itemize}

\newpage
\section{Standard shell history features}

In this section, we describe common shell history features. These features are most likely available to the majority our target users. Standard shell history features shape how people use shell history.  advantages and disadvantages


%We only focus on commonly used features. Most of the history features we describe are enabled by default. The list of history features we describe is following:
...
%Shells offer two editing modes which change the behavior and keybindings for some of the history features. We only cover the default Emacs editing mode.
%Most of the history features are either present in both modes or they have an equivalent feature in Vi mode.



\redtext{Fuck vim mode because it does not seem to be common enough - as in many people who use vim as an editor still use the default emacs mode in shell.}
